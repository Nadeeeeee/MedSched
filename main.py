...
class win():
    def __init__(self,rt):
        ...
        def searchWindow():
            ...
            searchmedi.geometry("500x300")
            searchmedi.resizable(width=False, height=False)

            searchmediFrame=LabelFrame(searchmedi, text="Medicine Name")
            searchmediFrame.pack(padx=10,pady=10)

            eSearchmedi =Entry(searchmediFrame)
            eSearchmedi.pack(padx=30, pady=30)

            srchBtn= Button(searchmedi, text="Search Medicine", command=searchFunction)
            srchBtn.pack(padx=30, pady=30)
        def addMed():
            conn = sqlite3.connect('DBmedsched')
            c = conn.cursor()
            confirm = messagebox.askquestion("Confirmation", "Do you want to add this medicine?")
...
#buttons
        ...
        about_us = Button(mainFrame, font=('arial', 10, 'bold'), text="About us", pady =1,padx=1, command=about_us)
        about_us.place(x=1, y=672)
        searchBtn = Button(mainlabelFrame,font=('arial', 18, 'bold'), text="Search Medicine", pady =1, bg='pink', command=searchWindow)
