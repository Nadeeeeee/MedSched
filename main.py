...
class win():
    def __init__(self,rt):
        ...
        self.rt.geometry("1200x730")
        self.rt.resizable(width=False, height=False)


        def searchNameFunction():
            namesearch = eSearchmedi.get()
            searchmedi.destroy()
            for recv2 in self.tbl_med.get_children():
                self.tbl_med.delete(recv2)
            conn = sqlite3.connect('DBmedsched')
            c = conn.cursor()
            c.execute("SELECT rowid, * FROM DBmedsched WHERE MedName like?", (namesearch,))
            rec = c.fetchall()
            global count
            count = 0
            for data in rec:
                if count % 2 == 0:
                    self.tbl_med.insert(parent='', index='end', iid=count, text='',
                                        values=(data[0], data[1], data[2], data[3], data[4], data[5], data[6]))
                else:
                    self.tbl_med.insert(parent='', index='end', iid=count, text='',
                                        values=(data[0], data[1], data[2], data[3], data[4], data[5], data[6]))
                count += 1
                messagebox.showinfo("Search Complete", "Search Complete, Refresh to go back to normal table")
            conn.commit()
            conn.close()
        def searchstrDFucntion():
            namesearch = eSearchmedi.get()
            searchmedi.destroy()
            for recv2 in self.tbl_med.get_children():
                self.tbl_med.delete(recv2)
            conn = sqlite3.connect('DBmedsched')
            c = conn.cursor()
            c.execute("SELECT rowid, * FROM DBmedsched WHERE StartDate like?", (namesearch,))
            rec = c.fetchall()
            global count
            count = 0
            for data in rec:
        ...
        def searchUntilFunction():
            ...
            conn = sqlite3.connect('DBmedsched')
            c = conn.cursor()
            c.execute("SELECT rowid, * FROM DBmedsched WHERE Until like?", (namesearch,))
            rec = c.fetchall()
            global count
            count = 0
            for data in rec:
        ...
        def searchIntkDate():
            ...
            conn = sqlite3.connect('DBmedsched')
            c = conn.cursor()
            c.execute("SELECT rowid, * FROM DBmedsched WHERE IntakeDays like?", (namesearch,))
            rec = c.fetchall()
            global count
            count = 0
            for data in rec:
        ...
        def searchIntkTime():
            ...
            conn = sqlite3.connect('DBmedsched')
            c = conn.cursor()
            c.execute("SELECT rowid, * FROM DBmedsched WHERE IntakeTime like?", (namesearch,))
            rec = c.fetchall()
            global count
            count = 0
            for data in rec:
        ...
        def searchAmt():
            ...
            conn = sqlite3.connect('DBmedsched')
            c = conn.cursor()
            c.execute("SELECT rowid, * FROM DBmedsched WHERE Amount =?", (namesearch,))
            rec = c.fetchall()
            global count
            count = 0
            for data in rec:
        ...
        def searchWindow():
            global eSearchmedi, searchmedi
            searchmedi = Toplevel(rt)
            searchmedi.title("Search Medicine")
            searchmedi.geometry("500x500")
            searchmedi.resizable(width=False, height=False)

            searchmediFrame=LabelFrame(searchmedi, text="Medicine")
            searchmediFrame.pack(padx=10,pady=10)

            eSearchmedi =Entry(searchmediFrame)
            eSearchmedi.pack(padx=30, pady=30)

            srchNameBtn= Button(searchmedi, text="Search Medicine by name", command=searchNameFunction)
            srchNameBtn.pack(padx=1, pady=1)
            srchStrD = Button(searchmedi, text="Search Medicine by Start Date", command=searchstrDFucntion)
            srchStrD.pack(padx=1, pady=1)
            srchu = Button(searchmedi, text="Search Medicine by Until Date", command=searchUntilFunction)
            srchu.pack()
            srcintD = Button(searchmedi, text="Search Medicine by Intake Day", command=searchIntkDate)
            srcintD.pack()
            scrintT = Button(searchmedi, text="Search Medicine by Intake Time", command=searchIntkTime)
            scrintT.pack()
            srchAmt = Button(searchmedi, text="Search Medicine by Amount", command=searchAmt)
            srchAmt.pack()
        def addMed():
            conn = sqlite3.connect('DBmedsched')
            c = conn.cursor()
        ...
        def Update_med():
            ...
            conn.close()
        def about_us():
            messagebox.showinfo("About us","This application was made by group 4. Members:Tristan Railey Tan, John lyndon Vasquez, Lance Pastrana, Chizel Diaz, April Nicole Martin")
...
#Entry
        ...
        eintkTime = Entry(mainlabelFrame, font=('courier', 20, 'bold'), width=30)
        eintkTime.grid(row=4, column=1)
        eAmt = Entry(mainlabelFrame, font=('courier', 20, 'bold'), width=30)
...
#buttons
        ...
        updateBtn= Button(mainlabelFrame,font=('arial', 20, 'bold'), text="Update", pady =1, padx=42,bg='yellow', command=Update_med)
        updateBtn.grid(row=1, column=2)
        deleteBtn = Button(mainlabelFrame,font=('arial', 20, 'bold'), text="Delete", pady =1,padx=47, bg='red', command=delete_med)
        deleteBtn.grid(row=2, column=2)
        refreshBtn = Button(mainlabelFrame,font=('arial', 18, 'bold'), text="Refresh Table", pady =1,padx=10, bg='#519259', command=display_med)
        refreshBtn.grid(row=3, column=2)
        selectBtn = Button(mainlabelFrame,font=('arial', 18, 'bold'), text="Select Medicine", pady =1, bg='#2D31FA', command=select_med)
        selectBtn.grid(row=4, column=2)
        about_us = Button(mainFrame, font=('arial', 10, 'bold'), text="About us", pady =1,padx=1, command=about_us)
        about_us.place(x=1, y=685)
        searchBtn = Button(mainlabelFrame,font=('arial', 18, 'bold'), text="Search Medicine", pady =1, bg='#FDF6EC', command=searchWindow)
        searchBtn.grid(row=5, column=2)
